...
inline int pt_get_page_permissions(uint32* directory, uint32 virtual_address )
{
    ...
    //panic("pt_get_page_permissions() is not implemented yet!");
	 ...
	 get_page_table(directory,virtual_address,&ABO_EID);
	 uint32 entry = ABO_EID[PTX(virtual_address)] & 0x00000FFF;
...
{
    uint32 dir_index = virtual_address >> 22;
    uint32 tbl_index = (virtual_address >> 12) & 0x3FF;

    uint32 pde = directory[dir_index];
    if ((pde & PERM_PRESENT) == 0) {
        cprintf("pt_clear_page_table_entry: Page table does not exist for VA=%x", virtual_address);
        return;
    }

    uint32 *page_table = (uint32 *)(KERNEL_BASE + (pde & 0xFFFFF000));

    page_table[tbl_index] = 0;
