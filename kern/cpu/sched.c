...
void sched_init_PRIRR(uint8 numOfPriorities, uint8 quantum, uint32 starvThresh) {
	{
		...
		quantums = (uint8*) kmalloc(sizeof(uint8) * num_of_ready_queues);
...
struct Env* fos_scheduler_PRIRR() {
	...
	return NULL;
#else
	panic("fos_scheduler_PRIRR: USE_KHEAP disabled!");
#endif
	//Comment the following line
...
void clock_interrupt_handler(struct Trapframe* tf) {
	if (isSchedMethodPRIRR()) {
		...
		for (int p = 1; p < num_of_ready_queues; p++) {
			LIST_FOREACH(curr_env, &ProcessQueues.env_ready_queues[p])
			{
				// starvation check
				if ((timer_ticks() - curr_env->waiting_ticks) >= star_th) {
					// remove from old queue
					sched_remove_ready(curr_env);
					curr_env->priority--;
...
void update_WS_time_stamps() {
	...
	if (p!=NULL){
		...
		LIST_FOREACH(e,&p->page_WS_list){
			uint32 va=e->virtual_address;
			int ta=pt_get_page_permissions(p->env_page_directory,va);
			uint32 used_bit=ta&PERM_USED;
			e->time_stamp=e->time_stamp>>1;
			if (used_bit){
				e->time_stamp=e->time_stamp|(1<<31);
				pt_set_page_permissions(p->env_page_directory, va, 0, PERM_USED);
			}
		}
	}
