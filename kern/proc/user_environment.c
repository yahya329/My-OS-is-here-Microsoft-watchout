...
//	panic("create_user_kern_stack() is not implemented yet...!!");
	...
	uint32* u_s = kmalloc(KERNEL_STACK_SIZE);



		if (u_s == NULL){
			panic("invalid to create user kernel stack");
		}
		pt_set_page_permissions(ptr_user_page_directory, (uint32)u_s, 0, PERM_PRESENT);


//		uint32 *f=NULL;
//
//		get_page_table(ptr_user_page_directory,(uint32)u_s,&f);
//		if (f==NULL)
//		{
//			panic("ssssss");
//		}
//		f[PTX(u_s)] &= ~PERM_PRESENT;
//
//
	return u_s;
//		cprintf("exit create_user_kern_stack");
